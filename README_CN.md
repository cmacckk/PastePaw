...
- 📋 **历史记录** - 自动保存您复制的所有内容。
- 📄 **文件** - 复制文件/文件夹，也能原样粘贴回去。
- 🖼️ **图片** - 粘回的图片保留透明通道（此前会丢失）。
- ✍️ **富文本** - 粗体、斜体与链接在往返后依然保留。
- 📁 **文件夹与收藏板** - 分类整理到文件夹；搜索到某条内容时可直接跳转到它所在的列表。
- 📌 **按 P 收藏** - 一个键把选中的剪贴存入「收藏」文件夹。
- 🏷️ **自定义标题** - 给剪贴起个自己的名字，并能按这个名字搜到它。
- 🗂️ **类型与来源筛选** - 按内容类型或按来源应用缩小列表。
- 🔍 **快速搜索** - 即使历史记录再多，也能秒级找到复制过的内容。
- 🔢 **粘贴第 N 条** - `Ctrl+1` 至 `Ctrl+9` 直接粘贴列表前几条。
- 🕘 **历史保留** - 自行设定保留多少天、多少条。
- 📤 **导出与导入** - 以可读的 JSON 文件在设备间迁移文件夹内容。
- 🔤 **字体** - 分别设置界面与内容的字体、粗细，以及整体字号。
