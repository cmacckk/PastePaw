...
| 项 | 状态 |
|---|---|
| 需求 3：窗口滑动动画（缓动 + 帧节拍） | ✅ 完成，待人工确认手感 |
| ⑥ `Ctrl+1~9` 快速粘贴 | ✅ 完成 |
| ① 内容类型筛选（标签页） | ✅ 完成 |
| ⑤ 按源应用筛选 | ✅ 完成（实现为筛选，非分组） |
| ④ Pinboard 差异补齐（卡片显示所属列表 + 点击跳转） | ✅ 完成 |
| ② 拖拽卡片到外部应用 | ❌ 按 Q4 决定不做 |
...
方案里把开发分为 v1.6 / v1.7 / v1.8 三个阶段，但**全部开发与验证是在同一个分支上一起完成的**
（包括人工验证也是整套一起验的）。因此**合成一次发布**，而不是拆成 4 次：
拆开会产生**从未被独立验证过**的中间版本，而且要多跑 3 次 CI、多 3 份 CHANGELOG。

**注意：文档里的“v1.6 / v1.7 / v1.8”是开发阶段名，不是已发布版本号。**
实际发布的版本是 **v1.6.0**，它包含文档里 v1.6–v1.8 加上两项新增需求（自定义标题、字体）的全部内容。

发版准备已完成：三处版本号同步（`Cargo.toml` / `tauri.conf.json` / `package.json`
+ `Cargo.lock`）、CHANGELOG 中英双语条目（标题就是 CI 要抽取的 `## v1.6.0`）、
中英 README 功能列表更新。

剩余步骤需由仓库所有者执行（本环境禁止代理向 `main` 提交）：合并分支 → 打 tag → 推送。 剩余为发版准备（版本号三处同步、CHANGELOG 中英双语、
README 功能列表尚未提及文件/富文本/导出导入/保留/筛选等新能力）。

#### 一个值得记下的环境诊断：“界面透明”

用户报告呼出后界面透明。查证结果：**不是回归**。

- `tauri.conf.json` 本身是 `transparent: true`；界面用 `bg-background/80` + `backdrop-blur` 绘制
- 用户设置的窗口特效是 `clear` → 代码调 `clear_mica()` **主动清除系统背景**
- ⇒ 透明窗口 + 无系统背景 + 80% 不透明度 = 看得到桌面。**这是 Clear 选项的设计表现**
- `settings.json` 的 `mica_effect: "clear"` 修改时间**早于本次工作**；切到 Mica Alt 后正常

附带纠正了两个我自己的错误假设：`updateSettings` 发的是**完整对象**（不会重置字段）；
设置存在 `settings.json` 而非 `settings` 表（表为空是正常的，它只是遗留读取路径）。

#### 动画：我先前写错的方向（已纠正）

方案原本写的是“从 Rust 逐帧 `SetWindowPos` → 前端 CSS `transform` 合成层”。**这是错的**：
窗口本身要从屏外滑入，**必须移动操作系统窗口**；固定在原位的大窗口里面做 CSS 变换动不了窗口，
而且窗口占着屏幕会挡住点击。CSS 只能动窗口**内**的内容。

实际修的是三个可证的缺陷（show/hide 两处相同）：

| 缺陷 | 原因 |
|---|---|
| 线性插值 | 每帧等量位移 → 机械感 |
| `sleep(10ms)` 实际睡 ~15.6ms | Windows 默认定时器粒度；15 帧实际约 234ms 而非 150ms，且取整不一致 → 抖动 |
| 每帧调 `window.hwnd()` | 多余的句柄查找 |

#### 验收标准之间的冲突（已按 A 处理）

“热键→可见 ≤ 80ms”与“动画 ≥ 55fps”**不可能同时成立**：55fps = 每帧 18ms，80ms 只有 4.4 帧，
4 帧不是动画而是“跳过去”。已按 **A** 处理：≤80ms 指“窗口开始出现”的延迟，
滑动本身 12 帧 × 15ms ≈ 180ms（与 Mac Paste 的 150–250ms 一致）。

#### 内容类型：只有 3 种

`CLIP_TYPES` 只有 `text` / `image` / `file`。
“链接”与“颜色”**不是剪贴类型**，捕获逻辑从不产出它们；
要做只能新增派生筛选或新增捕获路径（已选目前不做）。
